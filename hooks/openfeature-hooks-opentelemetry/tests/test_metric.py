...
def mock_get_meter(monkeypatch):
    ...
    mock_meter = Mock(spec=metrics.Meter, create_up_down_counter=side_effect, create_counter=side_effect)
...
def test_metric_before(mock_get_meter):
    ...
    mock_counters["feature_flag.evaluation.active_total"].add.assert_called_once_with(1, {
        "feature_flag.key": "flag_key",
        "feature_flag.provider.name": "test-provider",
    })
    mock_counters["feature_flag.evaluation.request_total"].add.assert_called_once_with(1, {
        "feature_flag.key": "flag_key",
        "feature_flag.provider.name": "test-provider",
    })
...
def test_metric_after(mock_get_meter):
    ...
    mock_counters["feature_flag.evaluation.success_total"].add.assert_called_once_with(1, {
        "feature_flag.key": "flag_key",
...
def test_metric_error(mock_get_meter):
    ...
    mock_counters["feature_flag.evaluation.error_total"].add.assert_called_once_with(1, {
        "feature_flag.key": "flag_key",
        "feature_flag.provider.name": "test-provider",
